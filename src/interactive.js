...
const INTERACTIVE_STATE_FILE = path.join(config.dataDir, 'interactive-state.json');
...
async function loadInteractiveState(fileStore) {
...
async function saveInteractiveState(state) {
...
function renderEvent(event, level = 'normal') {
...
async function runProjectTask(message, context) {
...
async function applyLastTurnResult(fileStore, state, { force = false, planOnly = false, interactive = false, confirm = null, projectService = null } = {}) {
...
export async function runInteractive({ bridge, fileStore, turnManager = null, projectService = null, projectPath = '' }) {
