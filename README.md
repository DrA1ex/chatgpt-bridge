...
ZIP, binary, and large artifacts keep the direct Blob/URL/`chrome.downloads` path and do not wait for preview UI when that direct capture succeeds. Text-like artifacts may use a two-step ChatGPT UI: clicking the filename opens a delayed preview either as a fullscreen `role="dialog"` shell or as a `[slot="content"]` library panel. The extension first binds the container to the exact expected filename, then finds the download action by stable metadata or a narrowly scoped multilingual `aria-label` fallback, waits through loader states, and closes the preview before processing the next artifact. A delayed preview from the previous file is closed and the current text-file card is retried once. The currently observed CodeMirror text preview also has a bounded UTF-8 DOM fallback. The retry always re-resolves the action from the live assistant turn after the preview closes; it never reuses a stale DOM node or serialized descriptor.
