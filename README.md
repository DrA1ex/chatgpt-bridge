...
Interactive workflows use one backend: the authenticated local Zipflow service. Open the current project workflow with:

```text
/workflow
```

The workflow surface exposes history, plan, diff, report, checks, repair, and preset configuration. There is no second interactive workflow engine or fallback routing. Normal project-aware prompts remain on the ordinary turn pipeline and attach the project ZIP directly to the user turn. `/task` makes the ZIP result mandatory, while `/chat` deliberately sends a direct prompt without project ZIP context.

`/apply` sends the selected result ZIP to Zipflow. If the project has no saved workflow yet, Bridge initializes the `apply-changes` preset automatically before opening the review surface. Zipflow owns archive inspection, project writes, checks, Git, deployment, history, rollback, and mutation idempotency.

The separate headless `bridge workflow init|validate|run|resume|discard|serve` CLI remains available for non-interactive operation. See `docs/WORKFLOWS.md` and `docs/ZIPFLOW_SERVER.md`.
...
`/apply [zipPath]` hands the selected or explicit ZIP to Zipflow and opens its advertised review/actions surface. `--plan` and `--interactive` stay on that review path and never dispatch an action by themselves; `--force` is rejected because interactive mutations must use actions advertised by Zipflow.
...
Enter `/workflow` in the interactive UI to open the current server-backed Zipflow workflow surface. Normal project-aware prompts use the ordinary turn pipeline and attach the current project ZIP to the same user message. Workflow subcommands never switch to another backend.

The server exposes three presets: **Apply changes from ChatGPT**, **Fix the project until checks pass**, and **Work through a task**. Zipflow owns all local mutations and advertises every action through the workflow surface; Bridge keeps ChatGPT orchestration and artifact selection separate.

The headless `bridge workflow ...` runner is independent from interactive slash commands. Full behavior is documented in `docs/WORKFLOWS.md`.
