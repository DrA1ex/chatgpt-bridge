...
class FakeBridge extends EventEmitter {
  ...
  async reloadExtension(options = {}) {
    ...
    return { accepted: { accepted: true }, reconnected: { extensionVersion: options.expectedVersion || '1.0.8' } };
  }
...
test('Setup page exposes extension-only diagnostics and authentication', async () => {
  ...
  try {
    ...
    assert.equal(statusBody.extensionCompatibility.recommendedExtensionVersion, '1.0.8');
...
test('real-browser E2E control endpoints preserve source identity and require URL-bound cleanup', async () => {
  ...
  try {
    ...
    const extensionReload = await fx.request('/browser/extension/reload', {
      method: 'POST',
      body: JSON.stringify({ sourceClientId: 'bootstrap-client', expectedVersion: '1.0.8', reloadTabs: true, timeoutMs: 25_000 }),
    });
    assert.equal(extensionReload.response.status, 200);
    assert.equal(extensionReload.body.reconnected.extensionVersion, '1.0.8');
    assert.deepEqual(fx.bridge.browserCalls[1], {
      type: 'reload-extension',
      options: { sourceClientId: 'bootstrap-client', expectedVersion: '1.0.8', reloadTabs: true, timeoutMs: 25_000 },
    });
