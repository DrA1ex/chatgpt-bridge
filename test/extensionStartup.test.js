...
test('startup extension reload does not ask when the connected bundle is already current', async () => {
  ...
  const result = await maybeReloadExtensionAtStartup({
    ...
    getHealth: async () => ({
      ...
      clients: [{ id: 'ext-1', ready: true, extensionVersion: '9.8.7', clientVersion: '7.6.5' }],
    }),
...
test('startup extension reload asks for confirmation and verifies reconnect version', async () => {
  ...
  const result = await maybeReloadExtensionAtStartup({
    ...
    mode: 'test',
    extensionDir: dir,
    getHealth: async () => ({ selectedClientId: 'ext-1', clients: [{ id: 'ext-1', ready: true, compatible: true, extensionVersion: '9.8.6', clientVersion: '7.6.4' }] }),
...
test('startup extension reload skips ask mode without an interactive confirmation channel', async () => {
  ...
  const result = await maybeReloadExtensionAtStartup({
    policy: 'ask',
    extensionDir: dir,
    getHealth: async () => ({ clients: [{ id: 'ext-1', ready: true, compatible: true }] }),
    confirm: async () => null,
    reload: async () => { reloaded = true; },
  });
  assert.equal(result.status, 'skipped');
  ...
  const result = await maybeReloadE2eExtension({
    ...
    api: async (_options, route, request = {}) => {
      ...
      if (route === '/browser/clients') {
        return { clients: [{ id: 'ext-e2e', ready: true, compatible: true, extensionVersion: '1.0.17' }], selectedClientId: 'ext-e2e' };
      }
      if (route === '/browser/extension/reload') {
        return { reconnected: { extensionVersion: '1.0.17' } };
      }
...
test('real E2E bootstraps an incompatible tab, reloads it, and selects the compatible reconnect', async () => {
  ...
  const api = async (_options, route, request = {}) => {
    ...
    if (route === '/browser/tabs/open') {
      ...
      return {
        client: {
          ...
          extensionVersion: '1.0.3', clientVersion: '3.0.3',
      ...
      return {
        clients: [reloaded
          ? {
              ...
              extensionVersion: '1.0.17', clientVersion: '3.0.17',
    ...
    if (route === '/browser/extension/reload') {
      ...
      return { reconnected: { id: 'updated-tab', extensionVersion: '1.0.17' } };
    }
