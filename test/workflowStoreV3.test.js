...
test('transition, immutable action payload, artifact, and workflow snapshot commit atomically', async (t) => {
  const target = await temporaryStore(t);
  const store = new WorkflowStore(target.root);
  ...
  await store.commitTransition('workflow-1', workflow, {
    ...
    actionPayloads: { action: { id: 'action', artifactKey: 'artifact' } },
    artifacts: { artifact: { id: 'artifact', status: 'verified' } },
  });
  assert.deepEqual(await store.getWorkflow('workflow-1'), workflow);
  assert.equal((await store.getActionPayload('action')).artifactKey, 'artifact');
  ...
  assert.equal(disk.actionPayloads.action.artifactKey, 'artifact');
