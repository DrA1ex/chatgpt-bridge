...
(() => {
  ...
  const CLIENT_ID_STORAGE_KEY = 'chatgptBridgeClientId';
  ...
  function loadConfig() {
    return {
      ...
      transport: String(gmGet('bridge.transport', DEFAULT_CONFIG.transport) || DEFAULT_CONFIG.transport),
  ...
  async function sendCritical(payload) {
    ...
    try {
      await postPollingPayloads([payload], { timeout: 10_000 });
      recordLocalLog('out.critical.sent', summarizePayload(payload));
      return true;
    } catch (err) {
      recordLocalLog('out.critical.failed', { type: payload?.type || 'unknown', error: err.message || String(err) });
      enqueuePollingPayload(payload, { priority: true });
      return false;
    }
  }
  ...
  function enqueuePollingPayload(payload, options = {}) {
    ...
    schedulePollingFlush(options.immediate !== false ? 0 : 80);
    return true;
  }

  function schedulePollingFlush(delayMs = 0) {
    if (pollingFlushInFlight) return;
    if (pollingFlushTimer) clearTimeout(pollingFlushTimer);
    pollingFlushTimer = setTimeout(() => {
      pollingFlushTimer = null;
      void flushPollingOutbox();
    }, delayMs);
  }

  async function flushPollingOutbox() {
    if (pollingFlushInFlight || !pollingOutbox.length || !CONFIG.token) return;
    pollingFlushInFlight = true;
    const batch = pollingOutbox.splice(0, 50);
    try {
      await postPollingPayloads(batch, { timeout: 20_000 });
      recordLocalLog('out.poll.batch', { count: batch.length, types: batch.map((item) => item?.type || 'unknown').slice(0, 8) });
    } catch (err) {
      pollingOutbox = batch.concat(pollingOutbox).slice(0, 1000);
      setPanelStatus('event send failed', err.message || String(err));
      recordLocalLog('out.poll.failed', { count: batch.length, error: err.message || String(err) });
    } finally {
      pollingFlushInFlight = false;
      if (pollingOutbox.length) schedulePollingFlush(CONFIG.reconnectMs);
    }
  }
  ...
  function sendPageStatus(type = 'page.status') {
    send({ type, url: location.href, title: document.title, time: Date.now(), session: getCurrentSession(), activeRequest: activeRequest ? publicRequestStatus(activeRequest) : null });
  }
  ...
  function connect() {
    ...
    if (CONFIG.transport === 'websocket') connectWebSocket();
  ...
  async function pollingLoop() {
    ...
    while (!pollAbort && CONFIG.transport === 'polling') {
      try {
        const url = httpUrl(`/tm/poll?clientId=${encodeURIComponent(getClientId())}`);
        const response = await gmRequestJson({ method: 'GET', url, timeout: 35_000 });
  ...
  function gmRequestJson({ method = 'GET', url, data = undefined, timeout = 30_000 }) {
      ...
      GM_xmlhttpRequest({
        ...
        onload(response) {
          if (response.status < 200 || response.status >= 300) {
            ...
            reject(new Error(`HTTP ${response.status}: ${typeof response.responseText === 'string' ? response.responseText.slice(0, 200) : ''}`));
            return;
          }
          recordLocalLog('http.response', { method, path: safeUrlPath(url), status: response.status });
          if (response.response && typeof response.response === 'object') resolve(response.response);
          else resolve(safeJsonParse(response.responseText) || {});
        },
        onerror() { recordLocalLog('http.failed', { method, path: safeUrlPath(url) }); reject(new Error(`Request failed: ${url}`)); },
        ontimeout() { recordLocalLog('http.timeout', { method, path: safeUrlPath(url), timeout }); reject(new Error(`Request timed out: ${url}`)); },
      });
    });
  }
  ...
  function initFloatingPanel() {
    ...
    root.innerHTML = `
      <style>
        ...
        #cgb-tab{position:absolute;right:0;bottom:0;transform:translateX(28px);transition:transform .25s ease;background:#111;color:#fff;border-radius:10px 0 0 10px;padding:10px 12px;box-shadow:0 4px 18px rgba(0,0,0,.25);cursor:pointer;user-select:none;display:flex;gap:8px;align-items:center}
        #cgb-dot{width:9px;height:9px;border-radius:50%;background:#fff;box-shadow:0 0 0 0 rgba(255,255,255,.7)}
      ...
      <div id="cgb-panel">
        ...
        <label>Transport</label><select id="cgb-transport"><option value="polling">HTTP polling</option><option value="websocket">WebSocket</option></select>
  ...
  function handleServerMessage(payload) {
    ...
    if (payload.type === 'ping') {
      send({ type: 'pong', time: Date.now(), url: location.href, title: document.title, session: getCurrentSession() });
      return;
    }
  ...
  async function handlePromptSend(payload) {
    ...
    try {
      await sendCritical({ type: 'prompt.accepted', requestId });
      ...
      send({ type: 'session.snapshot', requestId, session: getCurrentSession() });
      ...
      send({ type: 'status', requestId, status: 'sent' });
  ...
  function collectAndEmit(request) {
    ...
    const snapshot = readAssistantSnapshot(request.baselineAssistantCount);
  ...
  function readAssistantSnapshot(baselineCount) {
    const nodes = getAssistantNodes();
    if (!nodes.length) return { answer: '', thinking: '', raw: '', count: 0, format: 'none', artifacts: [] };
    const candidateNodes = nodes.length > baselineCount ? nodes.slice(baselineCount) : [nodes[nodes.length - 1]];
    const node = candidateNodes[candidateNodes.length - 1];
    if (!node) return { answer: '', thinking: '', raw: '', count: nodes.length, format: 'none', artifacts: [] };
    ...
    if (markdownNodes.length) {
      ...
      if (answer) return { answer, thinking, raw, count: nodes.length, format: 'markdown', artifacts };
    }
    ...
    return { answer, thinking, raw, count: nodes.length, format: contentNodes.length ? 'structured' : 'raw', artifacts };
  }
  ...
  async function handleArtifactFetch(payload) {
    ...
    try {
      ...
      if (!url && (artifact.kind === 'action' || artifact.kind === 'canvas')) {
        const materialized = await materializeArtifactAction(artifact);
  ...
  history.pushState = function bridgePushState() {
    ...
    setTimeout(() => sendPageStatus('page.changed'), 0);
    return result;
  };
  ...
  window.addEventListener('popstate', () => sendPageStatus('page.changed'));
